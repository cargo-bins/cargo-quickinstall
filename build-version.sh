...
if [ "$TARGET_ARCH" == "x86_64-unknown-linux-musl" ]; then
    ...
    sudo apt install binutils git cmake ninja-build python python3-distutils openjdk-8-jdk qemu-kvm binutils-aarch64-linux-gnu -y
    mkdir zigfolder
    ...
    echo 'linker = "/usr/bin/aarch64-linux-gnu-ld"' >>~/.cargo/config
