...
if [ "${ALWAYS_BUILD:-}" != 1 ] && curl_slowly --fail -I --output /dev/null "${REPO}/releases/download/${CRATE}-${VERSION}/${CRATE}-${VERSION}-${TARGET}.tar.gz"; then
    echo "${CRATE}/${VERSION}/${CRATE}-${VERSION}-${TARGET}.tar.gz already uploaded. Skipping."
    exit 0
fi
...
if [[ "$TARGET" == *"-linux-gnu"* ]]; then
    CARGO_TARGET="${TARGET}.${GLIBC_VERSION}"
fi
...
# shellcheck disable=SC2086
tar --format=v7 -c $BINARIES | gzip -9 -c >"${TEMPDIR}/${CRATE}-${VERSION}-${TARGET}.tar.gz"

echo "${TEMPDIR}/${CRATE}-${VERSION}-${TARGET}.tar.gz"
# FIXME: make a signal handler that cleans this up if we exit early.
if [ ! -d "${TEMPDIR:-}" ]; then
    TEMPDIR="$(mktemp -d)"
fi
