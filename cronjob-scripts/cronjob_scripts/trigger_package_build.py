...
def main():
    targets = get_targets()
    # FIXME: find a better type than tuple here. We mean list of target, list of crates and versions
    queues: list[(str, list[CrateAndMaybeVersion])] = []
    ...
    for target in targets:
        ...
        queues.append((target, queue))
        queues.append((target, popular_crates.copy()))

        if os.environ.get("RECHECK"):
            queues.append((target, [{"crate": "cargo-quickinstall"} ]))
    ...
    for index in range(1000):
        for target, queue in queues:
            triggered = trigger_for_target(target, queue, index)
            if triggered:
                triggered_count += 1
...
def trigger_for_target(target: str, queue: list[CrateAndMaybeVersion], index: int) -> bool:
    print(f"Triggering builds for {target}")

    if len(queue) <= index:
        return
    crate_and_maybe_version = queue[index]
    print(crate_and_maybe_version)
    ...
    version = get_current_version_if_unbuilt(
        repo_url=repo_url, crate=crate, target=target, requested_version=requested_version
    )
    if not version:
        return False
